...
BACKEND=/www/wwwroot/api-mq.zamzami.or.id
FRONTEND=/www/wwwroot/mq.zamzami.or.id
  ...
  conn.sftp((err, sftp) => {
    ...
    writeStream.on('close', () => {
      ...
      const cmd = `
        ...
        cd /www/wwwroot/api-mq.zamzami.or.id && npm install --omit=dev && npx pm2 start src/index.js --name api-mymq
        cd /www/wwwroot/mq.zamzami.or.id && npm install && npm run build && NODE_ENV=production PORT=3001 npx pm2 start server.js --name mymq-next
        npx pm2 save
      `;
